...
import { Truck, Search, Plus, Trash2, Edit2, Phone, AlertCircle, Wallet, History, CreditCard, ArrowUpRight, PlusCircle, Package, Users, X } from 'lucide-react';
...
const SupplierPage: React.FC<SupplierPageProps> = React.memo(({ data, setData, setCurrentPage, setDeepLinkData, deepLinkData, onClearDeepLink }) => {
 ...
 <div className="grid grid-cols-1 gap-2 md:p-5 text-right">
 <div className="bg-white p-3 md:p-4 rounded-[14px] md:rounded-xl md:rounded-2xl border border-slate-200 shadow-sm flex flex-row md:flex-col items-center md:justify-center text-right md:text-center gap-3 md:gap-0">
 ...
 <div className="text-[11px] sm:text-xs md:text-sm font-bold text-slate-500 uppercase mb-0.5 md:mb-1">إجمالي المديونية</div>
 ...
 <p className="hidden md:block text-xs text-slate-500 font-medium mt-2 leading-snug">إجمالي المبالغ المستحقة لجميع الموردين المسجلين</p>
 </div>
 </div>
 </div>
 ...
 className="bg-slate-900 text-white px-4 md:px-8 py-3.5 rounded-2xl font-bold flex items-center gap-2 hover:bg-slate-800 transition-all shadow-[0_4px_20px_rgb(0,0,0,0.05)] active:scale-[0.98] transition-all duration-200 shrink-0"
 >
 <Plus size={20} />
 ...
 className={cn(
"bg-white border border-slate-200/60 rounded-xl md:rounded-2xl p-4 md:p-5 lg:p-6 gap-3 md:gap-4 shadow-sm hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all relative overflow-hidden group border-t-4 border-t-emerald-500/20",
 ...
 className="p-2 bg-slate-50 hover:bg-slate-100 rounded-xl text-slate-500 hover:text-slate-600 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center shrink-0"
 >
 <Edit2 size={16} />
 </button>
 <button 
 onClick={() => setSupplierToDelete(supplier)}
 className="p-2 bg-red-50 hover:bg-red-100 rounded-xl text-red-300 hover:text-red-500 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center shrink-0"
 >
 <Trash2 size={16} />
 </button>
 </div>
 ...
 <div title="مورد منافس جداً" className="bg-emerald-500/10 p-2 rounded-full border border-emerald-500/20 cursor-pointer min-w-[44px] min-h-[44px] flex items-center justify-center shrink-0">
 <div className="w-3 h-3 bg-emerald-500 rounded-full animate-pulse" />
 </div>
)}
 <h3 className="font-black text-slate-900 text-xl">{supplier.name}</h3>
 </div>
 ...
 className={cn(
"flex-1 p-4 md:p-5 lg:p-6 rounded-3xl flex flex-col items-center justify-center cursor-pointer transition-all border-2",
 ...
 >
 <div className="text-[11px] sm:text-xs font-bold text-slate-500 uppercase mb-1">المستحق المالي</div>
 ...
 className="flex-1 p-4 md:p-5 lg:p-6 rounded-3xl flex flex-col items-center justify-center cursor-pointer transition-all border-2 bg-slate-50 border-slate-200/60 hover:border-slate-300 hover:bg-slate-100"
 >
 <div className="text-[11px] sm:text-xs font-bold text-slate-500 uppercase mb-1">المنتجات</div>
 <div className="text-xl font-black text-slate-900">{supplierProducts.length}</div>
 </div>
 </div>

 <div className="flex flex-wrap gap-2 justify-end">
 {(supplier.paymentMethods || []).map(method => (
 <span key={method} className="bg-slate-50/50 border border-slate-200/60/60 px-3 py-1.5 rounded-xl text-[11px] sm:text-xs font-bold text-slate-500 uppercase flex items-center gap-1">
 <CreditCard size={12} />
 ...
 <h2 className="text-2xl font-black text-slate-900 mb-4 flex items-center gap-2 justify-end leading-snug">
 منتجات المورد
 <Package className="text-emerald-500" />
 </h2>
 </div>

 <div className="flex-1 min-w-0 overflow-y-auto custom-scrollbar p-4 md:p-5 lg:p-6 pt-4 min-h-0">
 <div className="space-y-4">
 {productsToShow.map(p => (
 <div key={p.id} className="flex justify-between items-center p-3 bg-slate-50 rounded-xl md:rounded-2xl border border-slate-200/60 font-bold text-slate-900">
 <span>{p.name}</span>
 ...
 <button 
 onClick={() => setProductsToShow(null)}
 className="w-full py-4 bg-slate-900 text-white font-bold rounded-2xl transition-all shadow-[0_2px_10px_rgb(0,0,0,0.04)] active:scale-[0.98] transition-all duration-200"
 >
 إغلاق
 </button>
 </div>
 ...
 >
 <div className="p-4 md:p-5 lg:p-6 pb-0 shrink-0">
 <h2 className="text-2xl font-black text-slate-900 mb-8 flex items-center gap-2 justify-end leading-snug text-right">
 ...
 className="w-full bg-slate-50/50 border border-slate-200/60 rounded-2xl py-4 px-5 outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all font-black text-slate-900 text-right text-lg"
 ...
 className="w-full bg-slate-50/50 border border-slate-200/60 rounded-2xl py-4 px-5 outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all font-bold text-slate-900 text-right"
...
"flex items-center gap-2 px-5 py-3 rounded-xl md:rounded-2xl border-2 transition-all cursor-pointer font-bold text-xs uppercase tracking-tight",
 (supplierForm.paymentMethods || []).includes(method as any) 
 ?"border-emerald-500 bg-emerald-50 text-emerald-700 shadow-md" 
 :"border-slate-200/60 bg-slate-50 text-slate-500 hover:border-slate-200"
)}>
 <input 
 type="checkbox"
 className="hidden"
 ...
 className="flex-1 py-4 bg-slate-900 text-white font-bold rounded-2xl transition-all shadow-[0_4px_20px_rgb(0,0,0,0.05)] active:scale-[0.98] transition-all duration-200"
 >
 تأكيد المورد
 </button>
 </div>
