...
const stripAddressFieldLabel = (value: any, type?: 'block' | 'street' | 'building') => {
  ...
  if (!text) return '';
  text = text
    .replace(/^(?:قطعة|قطعه|block)\s*[:：#-]?\s*/i, '')
    .replace(/^(?:شارع|ش\.?|street|road)\s*[:：#-]?\s*/i, '')
    .replace(/^(?:منزل|بيت|مبنى|مبني|building|house|home)\s*[:：#-]?\s*/i, '')
    .trim();
  if (type === 'block') text = text.replace(/^(?:ق\.?|block)\s*[:：#-]?\s*/i, '').trim();
  if (type === 'street') text = text.replace(/^(?:ش\.?)\s*[:：#-]?\s*/i, '').trim();
  if (type === 'building') text = text.replace(/^(?:م\.?)\s*[:：#-]?\s*/i, '').trim();
  return text;
};
