...
export const SmartOfferModal: React.FC<SmartOfferModalProps> = ({ product, isOpen, onClose }) => {
 ...
 <span className="text-2xl md:text-3xl font-black text-amber-500 drop-shadow-sm">{discountPercent}%</span>
 </div>
...
"bg-rose-50/50 border-rose-100"
 ...
 <h4 className="font-black text-indigo-800 text-lg mb-2">تحدي المبيعات لتعويض الخصم 🎯</h4>
 {discountPercent === 0 ? (
 ...
 لكي تحافظ على نفس إجمالي الأرباح السابقة (التي كنت تحققها بدون خصم)، يجب أن تزيد حجم مبيعاتك من هذا الصنف بنسبة <span className="text-lg font-black bg-white px-2 py-0.5 rounded text-indigo-600 shadow-sm mx-1">{Math.ceil((targetSalesMultiplier - 1) * 100)}%</span> لتغطية تأثير نزول السعر.
 </p>
)}
 </div>
 </div>
