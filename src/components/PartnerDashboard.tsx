...
const GlobalStatBox = React.memo(({ label, value, color, icon: Icon, isPercent = false, subtext = '', unit = '' }: any) => {
 ...
 default: return 'from-slate-500/10 to-slate-500/5 text-slate-600 border-slate-200/60 shadow-slate-500/5';
 }
 }
 ...
 className={cn(
"p-4 md:p-5 lg:p-6 rounded-xl md:rounded-2xl border bg-white relative overflow-hidden group shadow-sm hover:shadow-[0_4px_20px_rgb(0,0,0,0.05)] transition-all duration-300 cursor-pointer active:scale-[0.98] transition-all duration-200",
 getGradient(color)
)}
 >
 ...
 <div className={cn(
"w-10 h-10 lg:w-12 lg:h-12 rounded-xl lg:rounded-2xl flex items-center justify-center bg-white shadow-[0_4px_20px_rgb(0,0,0,0.05)] shadow-current/10 border border-current/5 transition-transform group-hover:rotate-6 duration-300",
 ...
 <span className="text-[11px] sm:text-xs font-bold opacity-40 uppercase tracking-tighter">{unit}</span>
 <div className="text-xl lg:text-3xl font-black text-slate-900 tracking-tighter group-hover:scale-105 transition-transform origin-right">
 {isPercent ? `${value.toFixed(1)}%` : Number(value).toLocaleString('en-GB', { minimumFractionDigits: 3, maximumFractionDigits: 3 })}
 </div>
 </div>
 <div className="text-[11px] sm:text-xs font-bold text-slate-500 uppercase tracking-[0.1em] group-hover:text-slate-600 transition-colors">{label}</div>
 {subtext && (
...
const SectionHeader = ({ title, icon: Icon, color ="indigo", subtitle }: { title: string, icon: any, color?: string, subtitle?: string }) => (
 ...
 className={cn("p-4 md:p-5 lg:p-6 rounded-[1.25rem] md:rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.05)] ring-4 ring-opacity-20", 
 ...
 <h2 className="text-xl md:text-2xl lg:text-3xl font-black tracking-tighter text-slate-900">{title}</h2>
...
const PartnerDashboard: React.FC<PartnerDashboardProps> = ({ data, onNavigate, onLogout }) => {
 ...
 const bentoCardStyle ="bg-[#fdfbf7] p-5 md:p-6 rounded-3xl lg:rounded-[2rem] border border-[#f0e6d2] shadow-[0_4px_20px_-10px_rgba(212,192,152,0.3)] text-right relative overflow-hidden flex flex-col h-full";
 
 return (
 <div className="min-h-full bg-slate-50 p-4 md:p-6 lg:p-8 animate-in fade-in duration-500 transition-colors" dir="rtl">
 ...
 />
    ...
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="flex flex-col gap-3 mb-8">
        ...
        {isPushSupported && !pushEnabled && !pushDenied && (
          ...
          >
            <div className="flex items-center gap-3 text-right">
              <div className="bg-indigo-100 p-2 rounded-xl text-indigo-600 min-w-[44px] min-h-[44px] flex items-center justify-center shrink-0">
                <Bell size={20} className="animate-pulse" />
              </div>
              <div>
                <h4 className="text-[12px] font-bold border-b border-indigo-200/50 pb-1 mb-1 inline-block">تفعيل الإشعارات</h4>
                <p className="text-[11px] sm:text-xs sm:text-[11px] font-bold text-indigo-600/80 mt-0.5">احصل على تنبيهات فورية عند وصول طلبات جديدة</p>
              </div>
            </div>
            
            <button
              onClick={() => setShowPushModal(true)}
              className="bg-indigo-600 text-white px-5 py-2.5 rounded-xl text-xs font-bold shadow-[0_2px_10px_rgb(0,0,0,0.04)] shadow-indigo-200 flex items-center justify-center gap-2 w-full sm:w-auto hover:bg-indigo-700 hover:scale-[0.98] transition-all active:scale-[0.98] transition-all duration-200"
            >
          ...
          {showPushModal && (
            <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
                <motion.div
                    ...
                    className="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.08)] max-w-sm w-full text-center"
                >
                    <div className="w-16 h-16 bg-indigo-100 text-indigo-600 rounded-full flex items-center justify-center mx-auto mb-4">
                        <Bell size={32} />
                    </div>
                    <h3 className="text-xl font-black text-slate-900 mb-2">فعّل الإشعارات</h3>
                    <p className="text-slate-600 text-sm font-bold mb-6">لتصلك طلباتك الجديدة أول بأول حتى والتطبيق مغلق.</p>
                    <div className="flex flex-col gap-3">
                        <button
                          onClick={handleEnablePush}
                          disabled={isActivatingPush}
                          className="bg-indigo-600 text-white py-3 rounded-xl font-bold shadow-[0_2px_10px_rgb(0,0,0,0.04)] shadow-indigo-600/30 hover:bg-indigo-700 transition-colors"
                        >
                        ...
                        <button
                          ...
                          disabled={isActivatingPush}
                          className="text-slate-500 py-2 text-xs font-bold hover:text-slate-900 transition-colors"
                        >
                          ليس الآن
                        </button>
                    </div>
       ...
       <div className="flex items-center gap-2 bg-white/60 backdrop-blur-md p-1.5 rounded-xl md:rounded-2xl border border-slate-200/60 shadow-sm min-w-[44px] min-h-[44px] flex items-center justify-center shrink-0">
      ...
      "px-5 py-2 text-[11px] font-black rounded-xl transition-all",
       filter === f ?"bg-slate-900 text-white shadow-[0_2px_10px_rgb(0,0,0,0.04)]" :"text-slate-500 hover:bg-slate-100"
      )}
       >
       ...
       <div className="mb-8">
        ...
        <motion.div layout className="mb-12 bg-white rounded-3xl p-2 md:p-5 border border-slate-200 shadow-sm hover:shadow-md transition-shadow overflow-hidden relative z-10 min-w-[44px] min-h-[44px] flex items-center justify-center shrink-0">
          <button 
       ...
       <div className="mb-12">
       <h3 className="text-xl font-black text-slate-900 mb-6 tracking-tight flex items-center gap-2 flex-row-reverse justify-end pr-2">
       ...
       <button onClick={() => setActiveWidget('campaign')} className="bg-gradient-to-br from-[#1a1a2e] to-slate-900 border border-slate-800 p-3 md:p-4 rounded-2xl text-right flex flex-col items-end group hover:scale-[1.02] transition-transform shadow-[0_8px_30px_rgb(0,0,0,0.08)] relative overflow-hidden outline-none">
       ...
       <p className="text-[11px] sm:text-xs font-bold text-slate-500 leading-relaxed">ابتكر حملات ذكية مبنية على أرصدتك</p>
       </button>
       
       <button onClick={() => setActiveWidget('forecast')} className="bg-gradient-to-br from-[#1a1a2e] to-slate-900 border border-slate-800 p-3 md:p-4 rounded-2xl text-right flex flex-col items-end group hover:scale-[1.02] transition-transform shadow-[0_8px_30px_rgb(0,0,0,0.08)] relative overflow-hidden outline-none">
       ...
       <p className="text-[11px] sm:text-xs font-bold text-slate-500 leading-relaxed">رؤية مستقبلية دقيقة لأداء مبيعاتك</p>
       </button>
      
       <button onClick={() => setActiveWidget('menu')} className="bg-gradient-to-br from-[#1a1a2e] to-slate-900 border border-slate-800 p-3 md:p-4 rounded-2xl text-right flex flex-col items-end group hover:scale-[1.02] transition-transform shadow-[0_8px_30px_rgb(0,0,0,0.08)] relative overflow-hidden outline-none">
       ...
       <p className="text-[11px] sm:text-xs font-bold text-slate-500 leading-relaxed">تحليل ربحية وشعبية كل صنف</p>
       </button>
       ...
       <div key={p.id} className="flex justify-between items-center bg-white p-3 rounded-xl md:rounded-2xl border border-[#f0e6d2] hover:border-amber-400 transition-colors flex-row-reverse">
       <div className="flex items-center gap-2 flex-row-reverse">
       <div className="w-7 h-7 rounded-lg bg-amber-50 border border-amber-100 text-amber-600 flex items-center justify-center font-bold text-[11px] sm:text-xs">#{i+1}</div>
       <div className="font-bold text-slate-900 text-[11px] truncate max-w-[100px]">{p.name}</div>
       </div>
       <div className="bg-slate-50/50 border border-slate-200/60/60 text-slate-500 text-[8px] font-bold px-2 py-1 rounded-full">{p.sold} طلب</div>
       </div>
      )) : <div className="text-center py-4 text-slate-300 font-bold text-[11px] sm:text-xs">لا توجد بيانات</div>}
       </div>
       <button onClick={() => setActiveWidget('bestsellers')} className="mt-3 text-center text-[9px] font-bold text-amber-600 hover:text-amber-700 transition-colors">عرض القائمة الكاملة</button>
       </div>
       </div>
       </div>
       ...
       >
       <div className="p-3 md:p-4 md:p-5 bg-white border-b border-slate-200/60 flex items-center justify-between sticky top-0 z-20">
       ...
       <div className="p-3 md:p-4 md:p-5 max-h-[80vh] overflow-y-auto custom-scrollbar">
       ...
       {topProducts.map((p, i) => (
       <div key={p.id} className="flex justify-between items-center bg-white p-3 md:p-4 rounded-xl md:rounded-2xl border border-slate-200/60 shadow-sm flex-row-reverse">
       <div className="flex items-center gap-4 flex-row-reverse">
       ...
       <div className="text-[11px] sm:text-xs text-slate-500 font-bold uppercase">{p.category || 'عام'}</div>
       </div>
       </div>
       <div className="flex flex-col items-start gap-1">
       <div className="text-indigo-600 text-lg font-black">{p.sold} طلبات</div>
       <div className="text-[11px] sm:text-xs text-slate-300 font-bold">إجمالي المبيعات المباشرة</div>
       </div>
       </div>
      ))}
       </div>
       ...
       {activeWidget === 'menu' && (
       <div id="products-matrix-section" className="bg-gradient-to-br from-[#1a1a2e] to-[#16213e] rounded-2xl p-3 md:p-4 shadow-[0_4px_20px_rgb(0,0,0,0.05)] relative overflow-hidden" dir="rtl">
       ...
       <span className="text-[11px] sm:text-xs text-white/50 font-bold bg-white/5 px-2 py-1 rounded-md">حجم مبيعات عالي + ربح عالي</span>
       </div>
       ...
       <span className="text-[11px] sm:text-xs text-white/50 font-bold bg-white/5 px-2 py-1 rounded-md">حجم مبيعات عالي + ربح منخفض</span>
       </div>
       ...
       {/* Puzzles */}
       <div className="bg-white/5 border border-blue-500/30 rounded-2xl p-4 md:p-5 lg:p-6 relative overflow-hidden group hover:bg-white/10 transition-all">
       <div className="flex items-center justify-between mb-4">
       <div className="flex items-center gap-2 text-blue-400">
       <Search size={20} className="group-hover:scale-110 transition-transform" />
       <h4 className="font-black text-lg">الألغاز (Puzzles)</h4>
       </div>
       <span className="text-[11px] sm:text-xs text-white/50 font-bold bg-white/5 px-2 py-1 rounded-md">حجم مبيعات منخفض + ربح عالي</span>
       </div>
       ...
       {/* Turtle */}
       <div className="bg-white/5 border border-rose-500/30 rounded-2xl p-4 md:p-5 lg:p-6 relative overflow-hidden group hover:bg-white/10 transition-all">
       <div className="flex items-center justify-between mb-4">
       ...
       <span className="text-[11px] sm:text-xs text-white/50 font-bold bg-white/5 px-2 py-1 rounded-md">حجم مبيعات منخفض + ربح منخفض</span>
       </div>
       ...
       >
       <div className="h-full bg-gradient-to-br from-indigo-600 via-indigo-700 to-indigo-900 p-6 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] shadow-indigo-600/30 text-white relative overflow-hidden flex flex-col justify-between">
       ...
       <ShoppingBag size={40} />
       <div className="absolute -top-2 -right-2 bg-white text-indigo-600 text-[11px] font-bold w-7 h-7 rounded-full flex items-center justify-center shadow-[0_4px_20px_rgb(0,0,0,0.05)] border-2 border-indigo-500">
         {totalOrdersCount}
       </div>
       </div>
       </div>
       ...
       <div className="w-12 h-12 bg-white text-indigo-600 rounded-full flex items-center justify-center shadow-[0_2px_10px_rgb(0,0,0,0.04)] transform transition-transform group-hover:-translate-x-2 shrink-0 mr-4">
       ...
       >
       <div className="h-full bg-gradient-to-br from-slate-800 via-slate-800 to-slate-900 p-6 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] shadow-slate-900/30 text-white relative overflow-hidden flex flex-col justify-between border border-slate-700/50">
       ...
       <div className="w-12 h-12 bg-slate-700 text-white rounded-full flex items-center justify-center shadow-[0_2px_10px_rgb(0,0,0,0.04)] border border-slate-600 transform transition-transform group-hover:-translate-x-2 shrink-0 mr-4">
       <ChevronRight size={24} className="rotate-180" />
       </div>
       </div>
       </div>
