...
const OrderPage: React.FC<OrderPageProps> = ({
  ...
  const isReadOnly =
    ...
    isPaidStatus(selectedOrder?.status as string);
  ...
  useEffect(() => {
    if (selectedOrder) {
      ...
      const zone = (data?.zones || []).find(
        (z) => z.id === selectedOrder.regionId || z.name === zoneNameStr,
      );
      if (zone) {
        setOrderZoneId(zone.id);
      } else {
        setOrderZoneId(
          data.zones && data.zones.length > 0 ? data.zones[0].id : "",
        );
      }
    } else {
      setOrderZoneId("");
  ...
  useEffect(() => {
    ...
    const autoConvert = async () => {
      ...
      const pendingPaidOrders = data.orders.filter((o) => {
        ...
        const createdMs = createdRaw?.seconds
          ? createdRaw.seconds * 1000
          : new Date(createdRaw || 0).getTime();
      ...
      if (pendingPaidOrders.length > 0) {
        ...
        setTimeout(() => {
          setOrderDeliveryType(
            (orderToConvert as any).manuallyModifiedDeliveryType
              ? (orderToConvert as any).deliveryType || "company"
              : "company",
          );
          convertToInvoice({
            ...orderToConvert,
            deliveryType: (orderToConvert as any).manuallyModifiedDeliveryType
              ? (orderToConvert as any).deliveryType || "company"
              : "company",
          } as any);
        }, 1000);
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const matchesFilter =
      filterStatus === "all" ||
      (filterStatus === "today"
        ? isToday(order.date, order)
        : filterStatus === "failed"
          ? isFailedStatus(order.status)
          : filterStatus === "pending"
            ? (isPendingStatus(order.status) || isFailedStatus(order.status)) &&
              !String(order.status).includes("تجميع القطية") &&
              order.status !== "split_pending"
            : filterStatus === "split_pending"
              ? String(order.status).includes("تجميع القطية") ||
                order.status === "split_pending"
              : filterStatus === "paid"
  ...
  const hasUnselectedSuppliers = (order: Order) => {
    const unresolved = order.items.some((item) => {
      ...
      const productName =
        ...
        "منتج غير معروف";
      const supplierOptions = data.products.filter(
        (p) =>
          p.isActive !== false &&
          robustNormalize(p.name) === robustNormalize(productName),
      );
  ...
  const getStatusLabel = (status: string, order?: Order) => {
    ...
    if (isCancelledStatus(status)) {
      if (status === "انتهى وقت القطية" || status === "ملغي - انتهى وقت القطية")
        return "ملغي - انتهى وقت القطية";
      return "ملغي";
    }
    if (isPaidStatus(status)) {
      const isVerifiedPaid = order && (order as any).paymentStatus === "paid";
  ...
  const getOrderSubtotal = (order: Order) => {
      ...
      itemsTotal =
        (order.items || []).reduce((sum, item) => {
          ...
          if (!p) {
            ...
            if (productName) {
              p = (data?.products || []).find(
                (prod) =>
                  prod.isActive !== false &&
                  robustNormalize(prod.name) === robustNormalize(productName),
              );
            }
          }
    ...
    const storedAmount = Number(
      order.totalAmount || o.finalPrice || o.total || o.total_amount || 0,
    );
  ...
  const getOrderDeliveryFee = (
    ...
    const zone = (data?.zones || []).find(
      (z) =>
        z.id === (overrideZoneId || order.regionId) ||
        (!overrideZoneId && z.name === zoneNameStr),
    );
    const dCost = zone ? zone.cost : 1.0;
    const dProfit = zone ? zone.profit : 0;
    return zone && zone.finalPrice !== undefined
      ? zone.finalPrice
      : dCost + dProfit;
  };

  const getOrderDeliveryProfit = (
    order: Order,
    type: string,
    overrideZoneId?: string,
  ) => {
    ...
    try {
        ...
        import("../lib/haptics").then((m) => {
          ...
          const orderTotal = Number(
            getOrderSubtotal(order) +
              getOrderDeliveryFee(order, order.deliveryType || "company"),
          );
      ...
      if (
        ...
        const linkedInv = (data?.invoices || []).find(
          (inv) => inv.id === order.linkedInvoiceId && !inv.isDeleted,
        );
      ...
      setData((prev) => {
        ...
        if (
          ...
          updatedInvoices = prev.invoices.map((inv) =>
            inv.id === order.linkedInvoiceId
              ? { ...inv, paymentStatus: "paid", status: "مدفوعة" }
              : inv,
          );
        }
      ...
      if (selectedOrder?.id === orderId) {
        setSelectedOrder((prev) =>
          prev
            ? { ...prev, status: newStatus as any, ...additionalUpdates }
            : null,
        );
      }
    } catch (error) {
  ...
  const convertToInvoice = async (order: Order) => {
    ...
    try {
      ...
      const orderPhoneStr = order.customerPhone
        ? String(order.customerPhone).trim()
        : "";
      const orderNameStr = getOrderCustomerName(order)
        ? String(getOrderCustomerName(order)).trim()
        : "";

      console.log("Debug convertToInvoice: order =", order);

      const validPhone =
        orderPhoneStr.length > 0 &&
        orderPhoneStr !== "undefined" &&
        orderPhoneStr !== "null";
      const validName =
        orderNameStr.length > 0 &&
        orderNameStr !== "undefined" &&
        orderNameStr !== "null";

      // Prefer explicit customer ID if passed by mobile app and exists
      const existingById = order.customerId
        ? (data?.customers || []).find((c) => c.id === order.customerId)
        : undefined;

      const existingByName =
        !existingById && validName
          ? (data?.customers || []).find(
              (c) =>
                c.name &&
                normalizeArabic(String(c.name)) ===
                  normalizeArabic(orderNameStr),
            )
          : undefined;

      const existingByPhone =
        !existingById && validPhone && orderPhoneStr !== "00000000"
          ? (data?.customers || []).find(
              (c) => c.phone && String(c.phone).trim() === orderPhoneStr,
            )
          : undefined;
      ...
      if (matchedCustomer) {
        ...
        const currentMatchedName = matchedCustomer.name
          ? normalizeArabic(String(matchedCustomer.name))
          : "";
        const orderNameNormalized = validName
          ? normalizeArabic(orderNameStr)
          : "";

        if (validName && currentMatchedName !== orderNameNormalized) {
          console.log(
            `Updating customer name from ${matchedCustomer.name} to ${orderNameStr} (Phone match)`,
          );
        ...
        const fallbackName = validName
          ? orderNameStr
          : `عميل غير مسجل #${order.id.slice(-4)}`;
      ...
      console.log(
        "Debug convertToInvoice: targetCustomerId =",
        targetCustomerId,
      );
      console.log(
        "Debug convertToInvoice: newCustomerToAdd =",
        newCustomerToAdd,
      );
      ...
      const effectiveZoneId =
        order.id === selectedOrder?.id && orderZoneId
          ? orderZoneId
          : order.regionId;
      const zone = (data?.zones || []).find(
        (z) =>
          z.id === effectiveZoneId || (!orderZoneId && z.name === zoneNameStr),
      );

      const invoiceDeliveryFee = getOrderDeliveryFee(
        order,
        orderDeliveryType,
        effectiveZoneId,
      );
      const invoiceDeliveryProfit = getOrderDeliveryProfit(
        order,
        orderDeliveryType,
        effectiveZoneId,
      );
      ...
      const totalCost = (order.items || []).reduce(
        (sum, item) =>
          sum +
          (item.costAtTime ||
            (data?.products || []).find((p) => p.id === item.productId)?.cost ||
            0) *
            (item.quantity || 0),
        0,
      );
      const gatewayFee = 0.25;
      const subtotalAmount = getOrderSubtotal(order);
      const discountVal = (order as any).discount || 0;
      const totalAmount = Math.max(
        0,
        subtotalAmount + invoiceDeliveryFee - discountVal,
      );
      ...
      const isActuallyPaid =
        isPaidStatus(order.status) ||
        isPaidStatus((order as any).paymentStatus) ||
        (selectedOrder?.id === order.id && isMarkedAsPaid);
      if (!isActuallyPaid) {
        if (totalAmount > 0) {
          try {
            ...
            if (
              ...
              toast.error(
                "خطأ إنشاء الرابط: " +
                  (paymentData.message || paymentData.error || "غير معروف"),
              );
            }
          } catch (e) {
      ...
      const hasUnresolvedItems = order.items.some((it) => {
        ...
        const productName =
          product?.name || (it as any).name || (it as any).productName || "";
        const supplierOptions = data.products.filter(
          (p) =>
            p.isActive !== false &&
            robustNormalize(p.name) === robustNormalize(productName),
      ...
      if (hasUnresolvedItems) {
        toast.error(
          "يرجى تحديد المورد للأصناف متعددة الموردين قبل تحويل الفاتورة",
        );
        return;
      }
      ...
      setData((prev) => {
        ...
        const updatedProducts = prev.products.map((p) => {
          ...
          // Deduct addon stock
          if (
            Array.isArray(newP.addons) &&
            newP.addons.length > 0 &&
            Array.isArray(item.addons)
          ) {
            newP.addons = (newP.addons as any[]).map((addon) => {
              const selectedAddon = (item.addons as any[]).find(
                (a: any) => a.id === addon.id,
              );
              ...
              if (addon.calculationType === "fixed") addonQty = 1;
                ...
                addonQty = Math.ceil(
                  item.quantity / (addon.xItemsThreshold || 1),
                );
...
${paymentLink}`
    ...
    const promoCodeName =
      (order as any).appliedPromoCodeName ||
...
Alturath.kw`;
    ...
    if (
      (targetObj as any).splitType === "traditional" &&
      Array.isArray((targetObj as any).splitPayments)
    ) {
      const splitText =
        `\n\n*المشاركين بالقطية:*\n` +
        (targetObj as any).splitPayments
          .map(
            (sp: any) =>
      ...
      (targetObj as any).splitType === "roulette" &&
      Array.isArray((targetObj as any).splitParticipants)
    ) {
      const participants = (targetObj as any).splitParticipants
        .map((p: any) =>
          typeof p === "object"
          ...
          >
            {filterStatus === "pending" ? (
              <>
                ...
                <p className="text-emerald-500/80 font-bold max-w-sm mx-auto text-lg text-center">
                  لا توجد طلبات معلقة والعمليات تعمل بهدوء تام. استمتع بلحظات
                  النجاح الصافية.
                </p>
              </>
            ) : (
              <>
                ...
                <p className="text-slate-500 font-bold max-w-sm mx-auto">
                  لا توجد طلبات تطابق اختياراتك. جرب تغيير الفلتر.
                </p>
              </>
            )}
          </motion.div>
                  ...
                  <div className="lg:col-span-2 space-y-6 md:space-y-8">
                    ...
                    <div className="space-y-3 md:space-y-4">
                        ...
                        (selectedOrder as any).splitPayments.length > 0 && (
                          <div className="mb-4 bg-purple-50/50 border border-purple-100 p-3 md:p-4 rounded-xl">
                            ...
                            <div className="space-y-2">
                              {(selectedOrder as any).splitPayments.map(
                                (sp: any, i: number) => (
                                  <div
                                    key={i}
                                    className="flex justify-between items-center bg-white p-2.5 rounded-lg border border-purple-50 shadow-sm"
                                  >
                      ...
                      {(selectedOrder as any).splitType === "roulette" &&
                        Array.isArray(
                          (selectedOrder as any).splitParticipants,
                        ) &&
                          ...
                          <div className="mb-4 bg-purple-100 border-2 border-purple-400 p-4 rounded-xl shadow-inner relative overflow-hidden">
                            ...
                            <div className="bg-white rounded-xl p-3 border-2 border-purple-200 mb-3 text-center">
                              ...
                              <div className="text-base md:text-lg font-bold text-purple-700">
                                {(selectedOrder as any).rouletteLoser ||
                                  "غير معروف"}
                              </div>
                            </div>
                            ...
                            <div className="space-y-1">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {(selectedOrder as any).splitParticipants.map(
                                  (pName: any, idx: number) => {
                                    const pVal =
                                      typeof pName === "object"
                      ...
                      <h3 className="font-bold text-slate-800 flex items-center gap-2 text-lg md:text-xl">
                        <Package
                          size={20}
                          className="text-indigo-600 md:w-6 md:h-6"
                        />{" "}
                        الأصناف المطلوبة
                      </h3>
                                          ...
                                          !isReadOnly && (
                                            <select
                                              ...
                                              style={{
                                                ...
                                                backgroundPosition:
                                                  "left 0.5rem md:left 0.75rem center",
                                                backgroundSize:
                                                  "0.7rem md:0.8rem",
                                              }}
                                              value={
                                              ...
                                              onChange={async (e) => {
                                                ...
                                                if (selectedSupp) {
                                                  ...
                                                  toast.success(
                                                    `تم اختيار المورد: ${(data?.suppliers || []).find((s) => s.id === selectedSupp.supplierId)?.name || "غير معروف"}`,
                                                  );
                                                  ...
                                                  if (
                                                    ...
                                                    toast.info(
                                                      "جاري تحويل الطلب المدفوع إلى السجل تلقائياً...",
                                                    );
                                                    setTimeout(
                                                      () =>
                                                        convertToInvoice(
                            ...
                            <tfoot className="bg-slate-50/50 text-[10px] md:text-xs">
                              ...
                              {(selectedOrder as any).discount > 0 && (
                                <tr>
                                  <td
                                    colSpan={3}
                                    className="p-2 md:p-3 font-bold text-rose-500"
                                  >
                                    الخصم{" "}
                                    {(selectedOrder as any).appliedPromoCodeName
                                      ? `(${(selectedOrder as any).appliedPromoCodeName})`
                                      : ""}
                                  </td>
                              ...
                              <tr>
                                <td
                                  colSpan={3}
                                  className="p-3 md:p-4 font-bold text-slate-500"
                                >
                                  إجمالي الأصناف
                                </td>
                                <td className="p-3 md:p-4 text-left font-bold text-slate-700">
                                  {Number(
                                    getOrderSubtotal(selectedOrder),
                                  ).toFixed(3)}
                                </td>
                              </tr>
                              <tr>
                              ...
                              <tr className="bg-indigo-600 text-white">
                                <td
                                  colSpan={3}
                                  className="p-3 md:p-4 font-bold text-xs md:text-lg"
                                >
                        ...
                        <div
                          className={cn(
                            ...
                            isFailedStatus(selectedOrder.status as string)
                              ...
                              : String(selectedOrder.status).includes(
                                    "تجميع القطية",
                                  ) || selectedOrder.status === "split_pending"
                          ...
                          <button
                            onClick={async () => {
                              ...
                              if (!paymentLink || paymentLink.trim() === "") {
                                toast.info(
                                  "سيتم إنشاء رابط دفع جديد ثم فتح واتساب...",
                                );
                          ...
                          {false && (
                            <MagneticButton
                              ...
                              className={cn(
                                ...
                                !!(selectedOrder as any).paymentLink &&
                                  isMarkedAsPaid
                                  ? "cursor-not-allowed opacity-90 active:scale-100"
                                  : "",
                              )}
                            >
                            ...
                            <div className="text-xs md:text-sm font-bold leading-relaxed text-white/80">
                              {(() => {
                                ...
                                if (addr && typeof addr === "object") {
                                  ...
                                  if (addr.block)
                                    addrParts.push(`ق${addr.block}`);
                                  if (addr.street)
                                    addrParts.push(`ش${addr.street}`);
                                  if (addr.jaddah)
                                    addrParts.push(`ج${addr.jaddah}`);
                                  if (addr.building || addr.house)
                                    addrParts.push(
                                      `م${addr.building || addr.house}`,
                                    );
                                  if (addr.floor)
                                    addrParts.push(`الدور ${addr.floor}`);
                                  if (addr.apartment)
                                    addrParts.push(`شقة ${addr.apartment}`);
                                  if (addr.time) timeStr = addr.time;
                                } else {
                                  addrParts.push(
                                    addr ||
                                      (selectedOrder as any).area ||
                                      (data?.zones || []).find(
                                        (z) => z.id === selectedOrder.regionId,
                                      )?.name ||
                                      selectedOrder.regionId ||
                                      "غير محدد",
                                  );
                                }
                                ...
                                if (!timeStr) {
                                  ...
                                  if (ca) {
                                    if (ca.seconds)
                                      dateObj = new Date(ca.seconds * 1000);
                  ...
                  <button
                    onClick={() => {
                      if (setDeepLinkData)
                        setDeepLinkData({
                          search:
                            selectedOrder.linkedInvoiceId || selectedOrder.id,
                        });
                      setCurrentPage("invoices-list");
                    }}
