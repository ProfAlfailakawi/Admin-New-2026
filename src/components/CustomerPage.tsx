...
const CustomerPage: React.FC<CustomerPageProps> = React.memo(({ data, setData, deepLinkData, onClearDeepLink }) => {
 ...
 <div className="bg-white rounded-3xl p-3 md:p-3 border border-slate-200/60 shadow-sm">
 ...
 <div className="flex overflow-x-auto hide-scrollbar bg-slate-100 p-1.5 rounded-2xl gap-1 border border-slate-200/60 w-full sm:w-auto">
 {[
 ...
 className={cn(
"px-3 md:px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 whitespace-nowrap",
 ...
 className="bg-slate-100 border border-slate-200/60 rounded-2xl py-2.5 pr-10 pl-4 w-full text-xs font-bold text-slate-600 outline-none cursor-pointer hover:bg-slate-200 transition-all appearance-none"
 >
 ...
 <div className="overflow-x-auto rounded-3xl border border-slate-200/60">
 ...
 <h3 className="text-xl md:text-3xl font-bold text-slate-800 mb-3 tracking-tight">لا يوجد عملاء!</h3>
 ...
 className="bg-primary text-white hover:bg-primary/90 px-4 md:px-8 py-4 rounded-2xl font-bold flex items-center gap-2 shadow-xl shadow-primary/20 hover:-translate-y-1 transition-all active:scale-95 hover:rotate-1 mx-auto"
 >
 <Plus size={24} />
 ...
 shakingId === customer.id &&"bg-red-50/50 animate-[pulse_0.5s_ease-in-out_infinite]"
 )}
 ><td className="p-3">
 ...
 <span className="bg-accent/10 text-accent-dark text-[10px] font-bold px-2 py-0.5 rounded-full border border-accent/20 flex items-center gap-1">
 <Crown size={10} />
 VIP
 </span>
)}
...
"bg-red-100 text-red-700"
 ...
 <div className="bg-red-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-lg flex items-center gap-1 w-fit animate-pulse border border-red-800 shadow-md">
 <AlertCircle size={10} />
 ...
 <td className="p-3">
 <div className={cn(
"flex items-center gap-2 px-3 py-1 rounded-xl border text-[10px] font-bold w-fit",
...
<AnimatePresence>
 ...
 className="bg-white rounded-3xl w-full max-w-[95%] md:w-full md:max-w-2xl shadow-xl p-0 border border-slate-100 flex flex-col max-h-[90dvh] overflow-hidden"
 onClick={e => e.stopPropagation()}
 >
 <div className="p-3 md:p-3 pb-0 shrink-0">
 <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2 justify-end text-right">
 ...
 className="w-full bg-slate-50 border border-slate-200/60 rounded-2xl py-3 px-4 outline-none focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all font-bold text-slate-800 text-right font-mono"
...
<AnimatePresence>
 ...
 className="bg-white rounded-3xl md:rounded-3xl w-[95%] md:w-full max-w-2xl shadow-xl p-0 border border-slate-100 max-h-[90dvh] flex flex-col overflow-hidden"
 onClick={e => e.stopPropagation()}
 >
 ...
 <h2 className="text-xl md:text-2xl font-bold text-slate-800 leading-tight">سجل فواتير العميل</h2>
 <p className="font-bold text-primary text-sm">{selectedCustomerInvoices.name}</p>
 </div>
 <button onClick={() => setSelectedCustomerInvoices(null)} className="p-2 hover:bg-slate-100 rounded-xl transition-colors text-slate-500">
 <X />
 </button>
 </div>
 ...
 <div className="text-lg md:text-xl font-bold text-slate-900">{Number(inv.totalAmount || 0).toFixed(3)} د.ك</div>
 <div className="text-[10px] text-slate-500 font-bold uppercase">{inv.paymentMethod}</div>
 </div>
 <button
 onClick={(e) => {
 e.stopPropagation();
 ...
 <div className="text-right">
 <div className="font-bold text-slate-800 text-base md:text-lg">فاتورة #{inv.id}</div>
 <div className="text-[10px] md:text-xs text-slate-500 font-bold">{new Date(inv.date).toLocaleString('en-GB')}</div>
 </div>
 </div>
 <div className="flex flex-wrap gap-2 justify-end pt-4 border-t border-slate-200/60/50">
 ...
 <button 
 onClick={() => setSelectedCustomerInvoices(null)}
 className="w-full py-4 bg-slate-900 text-white font-bold rounded-2xl shadow-xl active:scale-95 transition-all"
 >
 إغلاق السجل
 </button>
 </div>
...
<AnimatePresence>
 ...
 className="bg-white rounded-2xl w-[95%] md:w-full max-w-lg shadow-xl p-0 border border-slate-100 text-right relative flex flex-col max-h-[90dvh] overflow-hidden"
 onClick={e => e.stopPropagation()}
 >
 <div className="p-3 pb-3 shrink-0 border-b border-slate-50 flex items-center justify-between flex-row-reverse">
 <h2 className="text-lg md:text-2xl font-bold text-slate-800 flex items-center gap-2 justify-end text-right">
 ...
 <p className="text-sm font-bold text-slate-800 mb-2">{analyzingCustomer.name}</p>
 ...
 <div className="text-[10px] md:text-[11px] text-emerald-600 font-bold mb-0.5 md:mb-1">صافي الربح التراكمي</div>
 ...
 <div className="text-[10px] md:text-[11px] text-sky-600 font-bold mb-0.5 md:mb-1">القيمة الحياتية (LTV)</div>
 ...
 className="w-full py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-500 transition-colors flex items-center justify-center gap-2 text-sm shadow-xl shadow-indigo-600/30"
 >
 ...
 className="w-full py-3 bg-amber-500 text-white font-bold rounded-xl hover:bg-amber-400 transition-colors flex items-center justify-center gap-2 text-sm shadow-xl shadow-amber-500/30"
 >
 ...
 className="w-full py-3 bg-blue-500 text-white font-bold rounded-xl hover:bg-blue-400 transition-colors flex items-center justify-center gap-2 text-sm shadow-xl shadow-blue-500/30"
 >
