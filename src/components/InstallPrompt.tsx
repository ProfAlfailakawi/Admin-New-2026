...
export default function InstallPrompt() {
  ...
  const handleInstall = async () => {
    if (!deferredPrompt) {
      ...
      toast.custom((t) => (
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-[0_4px_20px_rgb(0,0,0,0.05)] flex flex-col gap-2 font-bold text-sm min-w-[300px]" dir="rtl">
      ...
      {showPrompt && (
        <>
          ...
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 md:bottom-6 md:left-auto md:right-8 md:w-[420px] z-[100] bg-white rounded-t-[32px] md:rounded-3xl lg:rounded-[2rem] p-6 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] md:shadow-[0_20px_50px_rgba(0,0,0,0.15)] border-t md:border border-slate-200/60"
            dir="rtl"
          >
            ...
            <div className="flex items-start gap-4 mb-8">
              <div className="flex-shrink-0 w-[72px] h-[72px] bg-black rounded-[20px] flex items-center justify-center text-white shadow-[0_4px_20px_rgb(0,0,0,0.05)]">
                <span className="text-3xl sm:text-4xl font-black mb-1">ت</span>
              </div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex-1 bg-black hover:bg-slate-900 text-white flex items-center justify-center gap-2 py-4 rounded-[20px] font-bold text-base shadow-[0_2px_10px_rgb(0,0,0,0.04)] shadow-black/10 active:scale-[0.98] transition-all duration-200 transition-all duration-300"
              >
              ...
              <button
                onClick={handleDismiss}
                className="px-8 py-4 text-slate-500 font-bold text-base bg-slate-50 hover:bg-slate-100 rounded-[20px] active:scale-[0.98] transition-all duration-200 transition-all duration-300"
              >
                لاحقاً
              </button>
            </div>
