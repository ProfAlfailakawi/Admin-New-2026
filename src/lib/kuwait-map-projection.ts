 ...
 */
export function lonLatToKuwaitSvgPoint(longitude: number, latitude: number) {
 ...
 */
export function addStableJitter(point: {x: number, y: number}, seed: string | number = "", radius: number = 9) {
  let hash = 0;
 ...
 * ]
 */
export function getInvoicePoint(invoice: any, kuwaitAreas: any[]) {
 ...
 */
export function aggregateInvoicesByRegion(invoices: any[] = []) {
  const map = new Map();
