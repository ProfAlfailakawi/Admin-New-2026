...
export const computeAddonQuantity = (addon: any, item: any): number => {
    ...
    let multiplier = computeAddonSelectedQuantity(addon);
    if (multiplier <= 0) {
    ...
    if (addon.calculationType === 'fixed') {
        // Fixed addons apply a constant multiplier regardless of the item quantity (once per item selection/order)
        return multiplier;
    } else if (addon.calculationType === 'per_x_items') {
        const groups = Math.ceil(qty / threshold);
        return groups * multiplier;
    } else {
        // Default behaviour (per_item): one addon per item
