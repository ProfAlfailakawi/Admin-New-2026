 ...
 */
export async function generateQuickInstagramMessages(
  data: AppState,
  category: "motivation" | "engagement" | "promo",
) {
  ...
  try {
    ...
    let fallback =
      category === "motivation"
        ? [
            "النجاح يبي له طولة بال.. وإحنا معاك بكل خطوة 🌟",
            "خل يومك ناطع مثل حلوياتنا.. عيش اللحظة! ✨",
            "مو بس أكل.. إحنا نقدم لك سعادة في بوكس 🎁",
          ]
        : [
            "شنو بخاطرك اليوم؟ ناطع ولا خنين؟ 🤔",
            "منو رفيقك اللي دايماً يحلّي قعدتكم؟ منشنوه! 👇",
            "تراث بي.. طعم يذكّرك بأحلى الأيام 🏠",
          ];
...
export function simulateWhatIfScenario(
  ...
  if (invoices.length > 0) {
    ...
    if (sorted.length === 0)
      return {
        ...
        volumeImpact: 0,
        explanation: "لا توجد مبيعات مدفوعة مسجلة لإجراء تحليل دقيق.",
    ...
    currentMonthlyRevenue =
      totalRevenue * (timespanDays < 30 ? 1 : normalizationFactor);
    currentMonthlyProfit =
      totalProfit * (timespanDays < 30 ? 1 : normalizationFactor);
  ...
  const dataStatus: "sufficient" | "insufficient" =
    invoices.length > 0 ? "sufficient" : "insufficient";
  ...
  if (dataStatus === "insufficient") {
    ...
    if (product) {
      ...
      projectedMonthlyRevenue =
        currentMonthlyRevenue -
        productTotalRevenue * normalizationFactor +
        newProductRevenue;
      projectedMonthlyProfit =
        currentMonthlyProfit -
        (productTotalRevenue * normalizationFactor - productMonthlyCost) +
        newProductProfit;

      if (change > 0) {
        explanation =
          projectedMonthlyProfit > currentMonthlyProfit
        ...
        explanation =
          "السعر حالياً في نقطة التوازن بناءً على بياناتك التاريخية.";
      }
    }
 ...
 */
export function calculateBusinessHealthIndex(
  data: AppState,
): BusinessHealthScore {
  ...
  if (revPrev === 0) {
    ...
    revenueTrend =
      growth > 0.05 ? "improving" : growth < -0.05 ? "declining" : "stable";
  }
  ...
  const totalCustomersWithOrders = customers.filter(
    (c) => c.totalOrders > 0,
  ).length;
  const retentionRate =
  ...
  const highRiskSuppliers = supplierInsights.filter(
    (s) => s.riskLevel === "high",
  ).length;
  ...
  if (totalScore >= 75) {
    status = "Healthy";
    explanation =
      "نشاطك التجاري في وضع صحي ممتاز حالياً. العوامل الأساسية مستقرة وتشير إلى نمو مستدام.";
    recommendations.push(
      "استثمر الفوائض في توسيع قاعدة العملاء.",
      "حافظ على علاقاتك مع الموردين ذوي الكفاءة العالية.",
    );
  } else if (totalScore >= 50) {
    status = "Risk";
    explanation =
      "يوجد تذبذب في بعض المؤشرات الحيوية. يتطلب الأمر تدخلاً لمعالجة نقاط الضعف قبل تفاقمها.";
    recommendations.push(
      "راجع تكاليف التشغيل المخفية.",
      "قم بتفعيل حملة إعادة استهداف للعملاء المنقطعين.",
    );
  } else {
    status = "Critical";
    explanation =
      "تحذير: المؤشرات تشير إلى تراجع كبير واحتمالية عالية للخسارة. التدخل الفوري ضروري.";
    recommendations.push(
      "أوقف المنتجات غير المربحة فوراً.",
      "فاوض الموردين على شروط دفع أفضل لإنقاذ التدفق النقدي.",
    );
  }
  ...
  const finalScore: BusinessHealthScore = {
    ...
    factors: [
      {
        label: "ديناميكية الإيرادات",
        score: Math.round(revenueScore),
        weight: 25,
        trend: revenueTrend,
      },
      {
        label: "استقرار الأرباح",
      ...
      {
        label: "إدارة المخاطر",
        score: Math.round(riskImpactScore),
        weight: 15,
        trend: "stable",
      },
    ],
 ...
 */
export function generateSupplierNegotiationAnalysis(
  data: AppState,
): SupplierNegotiationInsight[] {
  ...
  products.forEach((product) => {
    ...
    const productInvoices = invoices
      .filter(
        (inv) =>
          !inv.isDeleted && inv.items.some((i) => i.productId === product.id),
      )
      .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());

    let costs = productInvoices
      .map((inv) => {
        const item = inv.items.find((i) => i.productId === product.id);
 ...
 */
export function generateRealProfitAnalysis(
  data: AppState,
): RealProfitInsight[] {
  ...
  products.forEach((product) => {
    ...
    productInvoices.forEach((inv) => {
      ...
      const priceSafe =
        item.priceAtTime !== undefined ? item.priceAtTime : product.price || 0;
      const costSafe =
        item.costAtTime !== undefined ? item.costAtTime : product.cost || 0;
      ...
      const deliveryLoss = Math.max(
        0,
        (inv.deliveryInfo?.cost || 0) - (inv.deliveryFee || 0),
      );
    ...
    const realProfitValue =
      rawProfit - totalGatewayFeesAllocated - totalDeliveryLossAllocated;
    const hiddenCostsRatio =
      totalRevenue > 0
        ? (totalGatewayFeesAllocated + totalDeliveryLossAllocated) /
          totalRevenue
        : 0;
  ...
  if (apiKey && apiKey !== "undefined" && apiKey !== "MISSING_API_KEY") {
    try {
      ...
      if (
        ...
        console.warn(
          "AI Quota Exceeded (429): Falling back to local campaign engine.",
        );
      } else {
        console.error(
          "AI Campaign Generation failed, falling back to local engine:",
          error,
        );
      }
    }
...
export function generateAIBusinessRecommendation(data: AppState): {
  ...
  const cacheKey = `ai-business-rec-v2-${generateStateHash(data)}`;
  ...
  const totalCost = activeInvoices.reduce(
  ...
  const highMargin = products.filter(
    (p) => p.price > 0 && (p.price - (p.cost || 0)) / p.price > 0.4,
  );
  ...
  if (vipChurn.length > 0) {
    finalResult = {
      title: "يا ولدي، ربعك القدامى وينهم؟",
      recommendation: `عندك ${vipChurn.length} نسايم خير من زباينك الـ VIP ما شفناهُم من شهر. طرش لهم هدية ولا خصم زقرتي يردهم لك، ترى العميل الوفي يسوى وزنه ذهب.`,
      type: "growth",
      iconType: "target",
    ...
    finalResult = {
      title: "صيدة اليوم يا النشمي",
      recommendation: `الصنف "${bestOne.name}" فيه بركة وهامش ربحه يطيب الخاطر. ركز إعلاناتك عليه هاليومين، وصدقني راح تلقى ربحك طاير فوق 20%.`,
      type: "growth",
      iconType: "stars",
    ...
    finalResult = {
      title: "دير بالك، الكيس فيه شق!",
      recommendation:
        "نفقاتك هالشهر تاكل أرباحك سكاتي. راجع موردينك واضغط عليهم شوي بالأسعار، و خفف من الأصناف اللي ما تمشي، الكاش ملك يا ولدي.",
      type: "operational",
      iconType: "shield",
    ...
    finalResult = {
      title: "أرضك خصبة، ازرع فيها الولاء",
      recommendation:
        "خيرك وايد والأمور طيبة بفضل الله. الحين وقت تكافئ زباينك، سو لهم نظام نقاط يخليهم ما يفارقونك ويجيبون ديرتهم لك.",
      type: "growth",
      iconType: "target",
    };
  }
...
export function generateStructuredCampaign(
  data: AppState,
  topic: string,
): AICampaign | null {
  const products = (data?.products || []).filter(
  ...
  const bestProduct =
    products.find((p) => p.id === bestProductId) || products[0];
  const now = new Date();
  ...
  if (topic === "زيادة الطلب") {
    ...
    campaign = {
      ...
      targetAudience:
        "العملاء المتواجدين في قواعد البيانات الخاصة بالطلبات المسائية.",
...
export function generateAILearningInsights(data: AppState): AILearningLog[] {
  ...
  if (customers.length > 0) {
    ...
    const activeRecently = customers.find((c) => {
      ...
      const noMiddle = !custInvs.some(
        (i) =>
          new Date(i.date) >= sixtyDaysAgo && new Date(i.date) < thirtyDaysAgo,
      );
...
export function generateHiddenRisks(data: AppState): HiddenRisk[] {
  ...
  const productStats = products
    .map((p) => {
      ...
      const revenue = soldItems.reduce(
        (s, i) => s + (i.priceAtTime || p.price || 0) * (i.quantity || 0),
        0,
      );
      const cost = soldItems.reduce(
        (s, i) => s + (i.costAtTime || p.cost || 0) * (i.quantity || 0),
        0,
      );
  ...
  const dangerousProducts = productStats.filter(
    (p) => p.qty > avgQty && p.margin < 15,
  );
  ...
  const customerStats = customers
    .map((c) => {
      ...
      const revenue = custInvoices.reduce(
        (s, inv) => s + (inv.totalAmount || 0),
        0,
      );
      const cogs = custInvoices
        .flatMap((i) => i.items || [])
        .reduce((s, i) => s + (i.costAtTime || 0) * (i.quantity || 0), 0);
  ...
  const topRevCustomers = sortRev.slice(
    0,
    Math.max(3, Math.floor(sortRev.length * 0.2)),
  );
  ...
  const revRecent = recentInvoices.reduce(
    (s, i) => s + (i.totalAmount || 0),
    0,
  );
  const revPrev = prevInvoices.reduce((s, i) => s + (i.totalAmount || 0), 0);
  const costRecent = recentInvoices
    .flatMap((i) => i.items || [])
    .reduce((s, i) => s + (i.costAtTime || 0) * (i.quantity || 0), 0);
  const costPrev = prevInvoices
    .flatMap((i) => i.items || [])
    .reduce((s, i) => s + (i.costAtTime || 0) * (i.quantity || 0), 0);

  const marginRecent =
    revRecent > 0 ? ((revRecent - costRecent) / revRecent) * 100 : 0;
  ...
  if (suppliers.length > 0) {
    const supplierStats = suppliers
      .map((s) => {
        ...
        const sInvoices = invoices
          .flatMap((inv) => inv.items || [])
          .filter((i) => sProducts.some((p) => p.id === i.productId));
        const sRev = sInvoices.reduce(
          (sum, item) => sum + (item.priceAtTime || 0) * (item.quantity || 0),
          0,
        );
        const sCost = sInvoices.reduce(
          (sum, item) => sum + (item.costAtTime || 0) * (item.quantity || 0),
          0,
        );
    ...
    const toxicSuppliers = supplierStats.filter(
      (s) => s.sMargin < 15 && s.sRev > totalRecentRev * 0.1,
    );
    if (toxicSuppliers.length > 0) {
      ...
      risks.push({
        ...
        iconType: "supplier",
        affectedProductNames: products
          .filter((p) => p.supplierId === worstSup.id)
          .map((p) => p.name),
      });
    }
  }
...
export function generateBusinessInsights(data: AppState): {
  ...
  if ((data?.suppliers || []).length > 0 && (data?.products || []).length > 0) {
    ...
    if (dependencyRatio > 0.4) {
      const supName =
        (data?.suppliers || []).find((s) => s.id === maxSupplierId)?.name ||
        "مورد غير معروف";
      const productsFromSupplier = (data?.products || [])
        .filter((p) => p.supplierId === maxSupplierId)
        .slice(0, 3);
  ...
  if ((data?.customers || []).length > 0) {
    const inactiveVIPs = (data?.customers || []).filter((c) => {
      ...
      const diffDays = Math.floor(
        (now.getTime() - lastOrder.getTime()) / (1000 * 3600 * 24),
      );
      return diffDays > 25; // Inactive for 25 days (stricter)
    });

    if (inactiveVIPs.length > 0) {
      const lostRevenue = inactiveVIPs.reduce(
  ...
  if (products.length > 0) {
    ...
    if (lowMarginProducts.length > 0) {
      insights.push({
        ...
        detailedPoints: [
          `أمثلة على هذه الأصناف: ${lowMarginProducts
            .slice(0, 2)
            .map((p) => p.name)
            .join("، ")}`,
  ...
  if (soldProducts.length > 0) {
    ...
    if (highestMarginProduct && highestMarginProduct.cost && totalRev > 0) {
      ...
      const margin = Math.round(
        ((highestMarginProduct.price - highestMarginProduct.cost) /
          highestMarginProduct.price) *
          100,
      );
...
export function generateAutoStrategies(data: AppState): AIStrategy[] {
  ...
  const inactiveVIPs = customers.filter((c) => {
    ...
    if (!c.lastActive) return true;
    const diffDays = Math.floor(
      (now.getTime() - new Date(c.lastActive).getTime()) / (1000 * 3600 * 24),
    );
    return diffDays > 30;
  });
  ...
  if (inactiveVIPs.length > 0) {
    ...
    strategies.push({
      ...
      title:
        "استراتيجية الاحتفاظ التكتيكي بالعملاء الاستراتيجيين (VIP Churn Recovery)",
  ...
  const productsWithMarginAndSales = products.map((p) => {
    ...
    const margin =
      p.price > 0 && p.cost > 0 ? ((p.price - p.cost) / p.price) * 100 : 0;
    return { ...p, sold, margin };
  });

  const highMarginLowSales = productsWithMarginAndSales.filter(
    (p) => p.margin > 50 && p.sold < 5 && p.isActive !== false,
  );

  if (highMarginLowSales.length > 0) {
    const targetProduct = highMarginLowSales.sort(
  ...
  const totalRevOverall = invoices.reduce(
    (s, i) => s + (i.totalAmount || 0),
    0,
  );
  const totalCostOverall = invoices
    .flatMap((i) => i.items || [])
    .reduce((s, i) => s + (i.costAtTime || 0) * (i.quantity || 0), 0);
...
export function performArchiveAnalysis(data: AppState) {
  ...
  if (
    !data ||
    (!data.testimonials && (!data.invoices || data.invoices.length === 0))
  ) {
    return {
      ...
      recommendations: [
        "لا توجد بيانات كافية لتوليد أية توصيات حقيقية، يرجى إدخال بيانات فعلية للعملاء ليتسنى للذكاء الاصطناعي تقديم التحليل.",
      ],
  ...
  reviews.forEach((r: any) => {
    ...
    keywords.positive.forEach((k) => {
      if (text.includes(k)) localPos++;
    });
    keywords.negative.forEach((k) => {
      if (text.includes(k)) localNeg++;
    });
    ...
    if (
      ...
      commonFeedback.push("تعليق يخص سرعة التوصيل");
    if (text.includes("لذيذ") || text.includes("طعم"))
      commonFeedback.push("تعليق يخص الطعم والجودة");
  });
  ...
  if (posCount > negCount && posCount > 0) {
    actualStrengths.push(
      `غالبية التقييمات تحمل طابع إيجابي (${sentimentScore}%)`,
    );
    if (
      commonFeedback.filter((f) => f === "تعليق يخص الطعم والجودة").length > 0
    )
      actualStrengths.push("توثيق لرضا العملاء بخصوص الطعم والجودة");
    if (commonFeedback.filter((f) => f === "تعليق حول التغليف").length > 0)
      actualStrengths.push("وجود إشادة واضحة بخصوص التغليف المعتمد");
  }

  if (actualStrengths.length === 0)
    actualStrengths.push(
      "لم يتم العثور على نقاط قوة محددة بوضوح في النصيات الحالية",
    );

  const actualWeaknesses: string[] = [];
  if (negCount > 0) {
    actualWeaknesses.push(
      `${negCount} إشارات بكلمات ذات طابع سلبي مسجلة في الآراء`,
    );
    if (
      commonFeedback.filter((f) => f === "تعليق يخص سرعة التوصيل").length > 0 &&
      negCount > posCount
    )
      actualWeaknesses.push(
        "احتمالية وجود ضعف في مدة أو آلية التوصيل بناءً على النصوص",
      );
  }

  if (actualWeaknesses.length === 0)
    actualWeaknesses.push("لا توجد إشارات سلبية واضحة مسجلة");
  ...
  if (reviews.length < 5) {
    actualRecs.push(
      `بيانات التقييمات ضعيفة جداً (${reviews.length} تقييم فقط)، ينصح بشدة بالبدء بحملة جمع تقييمات لاستخراج صورة أدق.`,
    );
  } else {
    if (negCount > posCount) {
      actualRecs.push(
        `بسبب غلبة الطابع السلبي، ينصح بالتواصل مع أصحاب التقييمات السلبية الأخيرة لسماع شكاواهم الحقيقية فوراً.`,
      );
    } else {
      actualRecs.push(
        `المحافظة الاستراتيجية على جودة العناصر الأكثر تكراراً في آراء العملاء: ${topRepeatedClean.join(", ")}.`,
      );
    }
    actualRecs.push(
      `تخصيص الخصومات أو الهدايا للعملاء الموثقين ضمن قائمة ה${reviews.length} مقیم الإيجابيين.`,
    );
  }
  ...
  const finalResult = {
    ...
    topRepeated:
      topRepeatedClean.length > 0
        ? topRepeatedClean
        : ["لا يوجد تكرار واضح للكلمات"],
...
export async function generatePulseArchiveAnalysis(
  allComments: string[],
): Promise<any> {
...
`;
    ...
    console.warn(
      "Pulse archive analysis external API failed or no key, falling back to local multi-layer categorization.",
      e,
    );
    ...
    const sortedTopics = Object.entries(topicsHeatmap)
      .sort((a, b) => b[1] - a[1])
      .map((x) => x[0]);
    const topKeywords = sortedTopics.slice(0, 4);
    if (topKeywords.length === 0)
      topKeywords.push("تعليقات عامة", "تفاعل", "مراجعات", "نبض");
...
export function analyzeKuwaitiSentiment(text: string): KuwaitiSentimentResult {
  ...
  const positiveLabels = [
    "شغل عدل (أعلى درجات الرضا)",
    "نطاعة وتميز (إيجابي جداً)",
    "ولا غلطة (ممتاز)",
    "إرضاء تام (تقييم عالي)",
  ];
  ...
  const negativeLabels = [
    "جرس إنذار (استياء)",
    "تدخل سريع (تقييم سلبي)",
    "مشكلة تشغيلية (انتباه)",
    "استياء تام (مراجعة)",
  ];
  ...
  const contextRules = [
    ...
    {
      patterns: ["عادي", "يمشي الحال", "نص ونص", "مو بطال"],
