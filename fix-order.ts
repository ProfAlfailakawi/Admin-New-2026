...
const oldContent = `  window.open(\`https://wa.me/\${phoneSafe}?text=\${encodeURIComponent(actionMsg)}\`,"_blank");
  setAnalyzingCustomer(nullconst StatCard: React.FC<{ label: string; value: string | number; icon: React.ReactNode; color: string; description?: string }> = ({ label, value, icon, color, description }) => {
...
const newContent = `  window.open(\`https://wa.me/\${phoneSafe}?text=\${encodeURIComponent(actionMsg)}\`,"_blank");
