...
export const installLocalStorageDataGuard = () => {
  ...
  // Defer storage optimization to idle time — avoids blocking the main thread
  // synchronously before React even renders. The guard prototype patches below
  // are what actually matter at boot; the optimization is housekeeping only.
  const _ric = (typeof window !== 'undefined' && (window as any).requestIdleCallback) || null;
  if (_ric) _ric(() => optimizeAndShrinkStorage(), { timeout: 6000 });
