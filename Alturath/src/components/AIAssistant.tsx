...
- عدد المنتجات: ${statsSummary.productCount}
- عدد الفواتير: ${statsSummary.invoiceCount}
- عدد العملاء المسجلين: ${statsSummary.customerCount}
- أشهر المنتجات: ${statsSummary.topProducts || 'لا يوجد'}

اكتب الرد بالعربية، بأسلوب كويتي مهني، واستخدم Markdown واضح ومنظم.`;
...
${cleanMessage}`;
 ...
 const response = await fetch('https://us-central1-gen-lang-client-0579828362.cloudfunctions.net/api/api/ai/generate', {
 method: 'POST',
 ...
 if (!response.ok) throw new Error("AI server failed");

 const aiData = await response.json();
 const aiText = aiData.text || aiData.result || 'عذراً يا طويل العمر، حصل خلل فني بسيط في التحليل. جرب مرة ثانية.';
