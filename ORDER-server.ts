...
// Disabled by default: this path double-notified every recipient.
//
// The Admin server owns payment alerting. It claims pushEvents ids that carry the
// item's birth moment as a suffix (`safe-worker-invoice-paid-INV-5097@202608291503`,
// added so a recycled invoice number cannot inherit an older claim), while every
// eventId built below is the bare base id (`safe-worker-invoice-paid-INV-5097`).
// Two different ids means neither atomic claim can see the other, so both servers
// sent — INV-5097 went out twice to all 6 devices (12 deliveries) and every recipient,
// not just one phone, got the message twice. Invoices 5093-5096 show a single send
// because this direct-push path is newer than they are.
//
// Set ORDER_SERVER_DIRECT_PUSH=1 to re-enable (only if the Admin server ever stops
// covering these alerts — and give it the Admin's era suffix first, or the duplicate
// comes straight back).
...
async function sendAdminPushDirectOnce(input: {
  ...
  if (!ORDER_SERVER_DIRECT_PUSH_ENABLED) {
    return { success: false, skipped: true, reason: "disabled-admin-server-owns-payment-alerts" };
  }
